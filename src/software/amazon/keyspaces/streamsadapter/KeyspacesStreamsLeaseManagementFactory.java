...
public class KeyspacesStreamsLeaseManagementFactory extends DynamoDBLeaseManagementFactory {
     ...
     */
    public KeyspacesStreamsLeaseManagementFactory(@NonNull KinesisAsyncClient kinesisClient,
        ...
        super(
                ...
                retrievalConfig.streamTracker().isMultiStream() ? new DynamoDBMultiStreamLeaseSerializer() : new DynamoDBLeaseSerializer(),
    ...
    public ShardDetector createShardDetector(StreamConfig streamConfig) {
        return new KeyspacesStreamsShardDetector(
                (AmazonKeyspacesStreamsAdapterClient) super.getKinesisClient(),
    ...
    // Overriding the lease cleanup manager as we dont have child shards yet and we will be doing lease cleanup in ShardSyncer.
    @Override
     ...
     * Since we are cleaning up the leases from {@link KeyspacesStreamsShardSyncer} we dont want
     * to run this thread to encounter breaking behaviour
     */
    static class NoOpLeaseCleanupManager extends LeaseCleanupManager {
        public NoOpLeaseCleanupManager(
