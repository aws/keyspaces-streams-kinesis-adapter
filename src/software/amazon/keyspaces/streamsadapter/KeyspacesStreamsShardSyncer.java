...
import software.amazon.keyspaces.streamsadapter.util.StreamsLeaseCleanupValidator;
...
import static software.amazon.keyspaces.streamsadapter.util.KinesisMapperUtil.MIN_LEASE_RETENTION_DURATION_IN_HOURS;
import static software.amazon.keyspaces.streamsadapter.util.KinesisMapperUtil.getShardCreationTime;
import static software.amazon.kinesis.common.HashKeyRangeForLease.fromHashKeyRange;

/**
 * A shard syncer implementation specifically for Keyspaces Streams that extends the Kinesis hierarchical shard syncer.
 * This class handles synchronization of shard and lease states for Keyspaces streams in Lease Table
 * (Lease Management). It is responsible for ensuring that the leases table is up-to-date with the shards
 * in the stream.
 */
public class KeyspacesStreamsShardSyncer extends HierarchicalShardSyncer {
    private static final Log LOG = LogFactory.getLog(KeyspacesStreamsShardSyncer.class);
    ...
    /**
     * Constructs a KeyspacesStreamsShardSyncer for single stream mode.
     */
    public KeyspacesStreamsShardSyncer() {
        this(false, "SingleStreamMode");
    }

    /**
     * Constructs a KeyspacesStreamsShardSyncer that can operate in either single or multi-stream mode.
     *
     ...
     */
    public KeyspacesStreamsShardSyncer(final boolean isMultiStreamMode, final String streamIdentifier) {
        this(isMultiStreamMode, streamIdentifier, null);
    }

    /**
     * Constructs a KeyspacesStreamsShardSyncer with support for deleted stream tracking.
     *
     * @param isMultiStreamMode Whether the syncer should operate in multi-stream mode
     * @param streamIdentifier The identifier for the stream being processed
     * @param deletedStreamListProvider Provider for tracking deleted streams
     */
    public KeyspacesStreamsShardSyncer(final boolean isMultiStreamMode,
     ...
     * @throws ProvisionedThroughputException If Keyspaces provisioned throughput is exceeded
    ...
    public synchronized boolean checkAndCreateLeaseForNewShards(@NonNull final ShardDetector shardDetector,
        ...
        syncShardLeases(shards, shardDetector, leaseRefresher, initialPosition, scope, ignoreUnexpectedChildShards, isLeaseTableEmpty);
        return true;
    }

    private void syncShardLeases(List<Shard> shards,
        ...
        LOG.info("syncShardLeases" + getStreamInfo()  + ": begin");
        LOG.debug("Num shards" + getStreamInfo()  + ": " + shards.size());
        ...
        LOG.debug("Num new leases to create" + getStreamInfo()  + ": " + newLeasesToCreate.size());
        for (Lease lease : newLeasesToCreate) {
            ...
            try {
                ...
                if (isMultiStreamMode) {
                    MetricsUtil.addStreamId(scope, shardDetector.streamIdentifier());
                }
                MetricsUtil.addSuccess(scope, "NewLeaseCreated", success, MetricsLevel.DETAILED);
        ...
        cleanupLeasesOfFinishedShards(currentLeases, shardIdToShardMap, shardIdToChildShardIdsMap, trackedLeases, leaseRefresher, multiStreamArgs);
        long timeTakenMillis = System.currentTimeMillis() - startTimeMillis;
        MetricsUtil.addLatency(scope, "ShardSyncLatency for" + getStreamInfo(), timeTakenMillis, MetricsLevel.SUMMARY);
        LOG.info("syncShardLeases" + getStreamInfo()  + ": end");
    }
    ...
    List<Lease> determineNewLeasesToCreate(final List<Shard> shards,
        ...
        LOG.info("determineNewLeasesToCreate"+ getStreamInfo() +  ": begin");
        ...
        LOG.info("determineNewLeasesToCreate" + getStreamInfo() + ": done");
    ...
    private void cleanupGarbageLeases(List<Shard> shards,
                                      ...
                                      MultiStreamArgs multiStreamArgs) throws ProvisionedThroughputException, InvalidStateException, DependencyException {

        LOG.info("cleanupGarbageLeases" + getStreamInfo() +": begin");
        ...
        if (!garbageLeases.isEmpty()) {
            LOG.info("Found " + garbageLeases.size()
                    ...
                    + " keyspaces shards to pick up recent/latest shards from stream"
                    + getStreamInfo()
            );
            ...
            for (Lease lease : garbageLeases) {
                if (StreamsLeaseCleanupValidator.isCandidateForCleanup(lease, currentKinesisShardIds, isMultiStreamMode)) {
        ...
        LOG.info("cleanupGarbageLeases" + getStreamInfo() + ": done");
    }
    ...
    private synchronized void cleanupLeasesOfFinishedShards(Collection<Lease> currentLeases,
        ...
        LOG.info("cleanupLeasesOfFinishedShards"+ getStreamInfo() + ": begin");
        ...
        LOG.info("cleanupLeasesOfFinishedShards" + getStreamInfo() + ": done");
    }
    ...
    synchronized void cleanupLeaseForClosedShard(String closedShardId,
        ...
        if ((leaseForClosedShard != null)
            ...
            try {
                if (Instant.now().isBefore(getShardCreationTime(closedShardId).plus(MIN_LEASE_RETENTION_DURATION_IN_HOURS))) {
                    okayToDelete = false; // if parent was created within lease retention period, don't delete lease for parent
                }
            } catch (RuntimeException e) {
                LOG.info("Could not extract creation time from ShardId [" + closedShardId +"]" + getStreamInfo() );
                LOG.debug(e);
            }

            if (okayToDelete) {
                LOG.info("Deleting lease for shard " + SHARD_ID_FROM_LEASE_DEDUCER.apply(leaseForClosedShard, multiStreamArgs)
                        + " as it is eligible for cleanup - its child shard is check-pointed at SHARD_END for the stream"
                        + getStreamInfo()
                );
    ...
    synchronized void assertClosedShardsAreCoveredOrAbsent(Map<String, Shard> shardIdToShardMap,
        ...
        for (String shardId : shardIdsOfClosedShards) {
            ...
            if (Instant.now().isBefore(getShardCreationTime(shardId).plus(MIN_LEASE_RETENTION_DURATION_IN_HOURS))) {
                LOG.info("Delaying deleting Shard " + shardId + " till lease retention duration is reached." + getStreamInfo());
                continue; // if parent was created within lease retention period, don't delete lease for parent
            }
            if (shard == null) {
                LOG.info("Shard " + shardId + " is not present in stream" + getStreamInfo() + "anymore.");
                continue;
            }
    ...
    private String getStreamInfo() {
        return isMultiStreamMode ? " for stream " + streamIdentifier : "";
    }
